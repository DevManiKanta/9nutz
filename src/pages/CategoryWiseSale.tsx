...
//    */
//   const fetchCategories = async (opts?: { search?: string; q?: string; page?: number; per_page?: number }) => {
//     setLoading(true);
//     try {
//       const params: any = {};
//       // prefer opts.search, fallback to opts.q (keeps backward compatibility)
//       const searchValue = opts?.search ?? opts?.q ?? undefined;
...
//       // canonical server payload (res.data)
//       const payload = res?.data ?? null;

//       // Rows extraction:
//       // - Laravel style: payload.data.data (pagination) OR payload.data (array)
//       // - Or payload.data may itself be the array of rows (your sample)
//       // - Or payload.rows / payload.categories
//       // - Or payload could be an array directly
//       let rows: any[] = [];
...
//       // Pagination meta: look in sensible places (payload.data if pagination, else payload)
//       const metaCandidate = payload?.data && typeof payload.data === "object" && !Array.isArray(payload.data) ? payload.data : payload;
...
//   // initial load
//   useEffect(() => {
//     void fetchCategories({ search: "", page: 1, per_page: perPage });
//     // eslint-disable-next-line react-hooks/exhaustive-deps
//   }, []);

//   // debounce search — call backend search automatically when query changes
//   useEffect(() => {
...
//       setPage(1);
//       void fetchCategories({ search: query, page: 1, per_page: perPage });
//     }, SEARCH_DEBOUNCE_MS);
...
//     setPage(newPage);
//     await fetchCategories({ search: query, page: newPage, per_page: perPage });
//   };
...
//           <button onClick={openCreate} className="bg-indigo-600 hover:bg-indigo-700 text-white py-2 px-4 rounded flex items-center">
...
//         ) : categories.length === 0 ? (
//           // changed text per your request
//           <div className="p-6 text-sm text-slate-500">No products available.</div>
//         ) : (
