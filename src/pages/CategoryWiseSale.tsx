...
//   // Helper: derive a reasonable base to resolve relative image paths (only used if server returns relative paths)
//   const deriveStorageBase = (() => {
//     let cached: string | null = null;
//     return (sampleAbsoluteUrl?: string) => {
//       if (cached) return cached;
//       // If we have an absolute URL sample, use its origin + `/public/storage/`
//       if (sampleAbsoluteUrl && /^https?:\/\//i.test(sampleAbsoluteUrl)) {
//         try {
//           const u = new URL(sampleAbsoluteUrl);
//           cached = `${u.origin}/public/storage/`;
//           return cached;
//         } catch {
//           // fallthrough
//         }
//       }
//       // else try axios baseURL
//       try {
//         const rawBase: string | undefined = (api as any)?.defaults?.baseURL;
//         if (rawBase) {
//           let b = rawBase.endsWith("/") ? rawBase.slice(0, -1) : rawBase;
//           // remove trailing '/api' or '/api/v1' if present
//           b = b.replace(/\/api(\/.*)?$/i, "");
//           cached = `${b}/public/storage/`;
//           return cached;
//         }
//       } catch {
//         // ignore
...
//         cached = `${window.location.origin}/public/storage/`;
//         return cached;
//       }
//       // final fallback
//       cached = "/public/storage/";
//       return cached;
//     };
//   })();

//   // Normalize a server row into CategoryItem with absolute image URL in `image`
...
//     // Prefer explicit full URL fields from API
...
//         resolvedImage = raw;
//       } else {
//         // try to resolve relative path against axios baseURL or window.location
//         try {
...
//       if (rows.length) {
//         setCategories(rows.map((r, idx) => normalizeRow(r, idx)));
//       } else {
//         setCategories([]);
