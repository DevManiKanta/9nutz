...
const InventoryManager: React.FC = () => {
  ...
  // --- SEARCH & PAGINATION STATES (ADDED) ---
  const [searchTerm, setSearchTerm] = useState("");
  const [currentPage, setCurrentPage] = useState(1);
  const itemsPerPage = 8; // default per-page (change here if needed)
  ...
  // clamp currentPage when items/search results change
  useEffect(() => {
  ...
  async function saveItem(e?: React.FormEvent) {
    ...
    try {
      ...
      setForm(defaultForm);
      setFormErrors({});
      setCurrentPage(1); // show latest added on first page
    } catch (err: unknown) {
      const { message, details, status } = formatAxiosError(err);
      console.error("Save inventory error:", { message, status, details, raw: err });
  ...
  // -----------------------
  function getFilteredItems(list: Inventory[], term: string) {
    const q = term.trim().toLowerCase();
    if (!q) return list;
    return list.filter((it) => {
      // product id or name
      const pid = String(it.product_id ?? "").toLowerCase();
      const pname = String(getProductName(it.product_id)).toLowerCase();
      const vid = String(it.vendor_id ?? "").toLowerCase();
      const vname = String(getVendorName(it.vendor_id)).toLowerCase();
      const type = String(it.type ?? "").toLowerCase();
      const note = String(it.note ?? "").toLowerCase();
      const when = String(it.created_at ?? "").toLowerCase();
      return (
        pid.includes(q) ||
        pname.includes(q) ||
        vid.includes(q) ||
    ...
    <div className="p-6 max-w-6xl mx-auto">
      ...
      <div className="mb-4 flex flex-col md:flex-row md:items-center md:justify-between gap-3">
        <input
          ...
          placeholder="Search by product id/name, vendor id/name, type, note or date"
          className="w-full md:w-1/2 border rounded px-3 py-2 focus:ring focus:ring-indigo-200"
        />

        {/* Simple info on filtered count */}
      ...
      {filteredItems.length > 0 && (
        <div className="flex items-center justify-between mt-4">
          <div className="text-sm text-slate-600">
            Showing {filteredItems.length === 0 ? 0 : ( (currentPage - 1) * itemsPerPage + 1 )} - {Math.min(currentPage * itemsPerPage, filteredItems.length)} of {filteredItems.length}
          </div>
