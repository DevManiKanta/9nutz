...
//  * Config
//  */
// const PRODUCTS_API ="http://192.168.29.100:8000/api/admin/products/show"; 
// const ORDERS_API = "http://192.168.29.102:5000/api/orders"; 
...
//   const apiRoot = (() => {
//     try {
//       return PRODUCTS_API.replace("/admin/products/show", "");
//     } catch {
//       return PRODUCTS_API;
//     }
//   })();
...
//         const res = await fetch(PRODUCTS_API, { method: "GET", headers });
//         if (!res.ok) throw new Error(`Server ${res.status}`);
//         const body = await res.json().catch(() => null);
//         let rows: any[] = [];
//         if (Array.isArray(body)) rows = body;
...
//     if (token) headers.Authorization = `Bearer ${token}`;
//     const res = await fetch(`${apiRoot}/admin/products/add`, { method: "POST", headers, body: fd });
...
//       }
//     })();
//     if (!res.ok) throw new Error((body && (body.message || body.error)) || `Server error (${res.status})`);
...
//     const headers: Record<string, string> = {};
//     if (token) headers.Authorization = `Bearer ${token}`;
//     const res = await fetch(`${apiRoot}/admin/products/update/${id}`, { method: "POST", headers, body: fd });
//     const body = await (async () => {
//       try {
//         return await res.json();
//       } catch {
...
//     const headers: Record<string, string> = { Accept: "application/json" };
//     if (token) headers.Authorization = `Bearer ${token}`;
//     const res = await fetch(`${apiRoot}/admin/products/delete/${id}`, { method: "DELETE", headers });
//     let body = null;
//     try {
//       body = await res.json();
//     } catch {}
...
//               <div className="text-xs text-slate-400 mt-3">This is a demo checkout — the component will attempt to POST to <code className="bg-slate-100 px-1 rounded">/api/orders</code>.</div>
//             </div>
//           </div>
//         </aside>
//       </div>
    ...
    <div className="min-h-screen bg-gray-50 p-6">
      ...
      <div className="bg-white rounded-xl p-6 shadow-sm mb-6">
        <div className="flex items-center justify-between mb-4">
          <div className="text-lg font-medium">Products</div>
          <div className="flex items-center gap-2">
            <button onClick={() => void refreshProducts()} className="px-3 py-2 rounded border inline-flex items-center gap-2">
              <RefreshCw />
              Refresh
            </button>
            <button onClick={() => openAdminCreate()} className="px-3 py-2 rounded bg-emerald-600 text-white inline-flex items-center gap-2">
              <Plus />
              Add
            </button>
          </div>
