...
//             <h1 className="text-xl font-semibold">9Nutz Business </h1>
//           </div>
//         )}
...
export const DashboardSidebar: React.FC<SidebarProps> = ({ isCollapsed, onToggle }) => {
  ...
  return (
    <div
      className={cn(
        // 🎨 Light brown theme
        "bg-amber-100 text-gray-800 h-screen flex flex-col border-r border-amber-300",
        isCollapsed ? "w-16" : "w-64"
      )}
    >
      {/* header */}
      <div className="h-16 flex items-center justify-between px-4 border-b border-amber-300 flex-shrink-0">
        {!isCollapsed && (
          <div className="flex items-center gap-2">
            <div className="w-8 h-8 rounded-lg flex items-center justify-center overflow-hidden bg-amber-200" />
            <h1 className="text-xl font-semibold text-amber-900">9NUTZ</h1>
          </div>
        )}
      ...
      {!isCollapsed && (
        <div className="p-4 border-t border-amber-300 flex-shrink-0">
          <div className="flex items-center gap-3">
            ...
            <span className="text-sm text-amber-900">Logged in User</span>
          </div>
        </div>
      )}
